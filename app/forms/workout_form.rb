class WorkoutForm
  ...
  def update
    ...
    workout.update!(workout_date:, workout_title:, workout_time:, workout_weight:, repetition_count:, set_count:, workout_memo:, user_id:)
    body_part_ids.map(&:to_i).each do |body_part_id|
      next if WorkoutBodyPart.find_by(workout_id: workout_id, body_part_id: body_part_id)
      WorkoutBodyPart.create(workout_id: workout_id, body_part_id: body_part_id)
    end
  end
end
