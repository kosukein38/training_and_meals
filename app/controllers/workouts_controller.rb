class WorkoutsController < ApplicationController
  ...
  def index
    @workouts = Workout.includes(:workout_images_attachments,
                                 user: :avatar_attachment).order(workout_date: :desc).page(params[:page])
  end
  ...
  def workouts_feed
    @feed_items = Workout.includes(:workout_images_attachments,
                                   user: :avatar_attachment).where(user_id: [*current_user.following_ids]).order(workout_date: :desc).page(params[:page])
  end
